...
export const useAuthStore = defineStore("auth", () => {
  ...
  const hasPermission = (permission: string): boolean => {
    ...
    const rolePermissions: Record<string, string[]> = {
      admin: ['all', 'user:read', 'user:write', 'user:delete', 'settings:read', 'settings:write', 'system:read', 'system:write'],
      user: ['user:read'],
      hq_admin: ['all', 'operator:read', 'operator:write', 'operator:delete', 'site:read', 'tariff:read', 'tariff:write', 'analytics:read', 'audit:read', 'settings:read', 'settings:write']
    };

    // Admin and HQ Admin have all permissions
    if (user.value?.user_type === 'admin' || user.value?.user_type === 'hq_admin') {
      return true;
    }

    // Check if user's role has the requested permission
    return user.value && rolePermissions[user.value.user_type]
  ...
  async function login(credentials: LoginCredentials): Promise<void> {
    ...
    try {
      ...
      const allMockCreds = getMockCredentials();
      const mockCreds = matchedUser ? allMockCreds[matchedUser.user_type] : null;
