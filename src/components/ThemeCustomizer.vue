...
// Official Shadcn color schemes
const officialColorSchemes = [
...
const customColorSchemes = [
  ...
  { name: 'starbucks', value: 'starbucks', hsl: '161.74 25% 44%' },
];

// Combined color schemes
const colorSchemes = [...officialColorSchemes, ...customColorSchemes];
...
// Special function to apply the Starbucks theme comprehensively
function applyStarbucksTheme() {
  const root = document.documentElement;
  ...
  // Set data attribute for Starbucks theme
  root.setAttribute('data-color-scheme', 'starbucks');

  // Update font settings (handled by CSS, but we set body font here)
  document.body.style.fontFamily = "'Work Sans', system-ui, sans-serif";
  ...
  <Dialog v-model:open="isOpen">
    <DialogContent class="sm:max-w-[500px] p-6">
      ...
      <Tabs v-model:value="activeTab" class="w-full mt-4">
        ...
        <!-- Color Schemes Tab -->
        <TabsContent value="colors" class="p-4 space-y-4">
          <!-- Official Shadcn Themes -->
          <div>
            <h3 class="text-sm font-medium mb-2">Official Shadcn Themes</h3>
            <div class="grid grid-cols-3 gap-2">
              <TooltipProvider>
                <div v-for="color in officialColorSchemes" :key="color.value">
                  <Tooltip>
          ...
          <!-- Custom Themes -->
          <div>
            <h3 class="text-sm font-medium mb-2">Custom Themes</h3>
            <div class="grid grid-cols-3 gap-2">
              <TooltipProvider>
                <div v-for="color in customColorSchemes" :key="color.value">
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        variant="outline"
